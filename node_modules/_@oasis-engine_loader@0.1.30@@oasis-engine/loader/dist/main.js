...
var o3Draco = require('@alipay/o3-draco');
...
var BufferLoader = (_dec = o3Core.resourceLoader(o3Core.AssetType.Buffer, ["bin", "r3bin"], false), _dec(_class = /*#__PURE__*/function (_Loader) {
  ...
  _createClass(BufferLoader, [{
    ...
    value: function load(item) {
      ...
      if (isBase64(url)) {
        return new o3Core.AssetPromise(function (resolve) {
...
function createVertexElement(gltf, semantic, accessor, index) {
  var size = getAccessorTypeSize(accessor.type);
  return new o3Core.VertexElement(semantic, 0, getElementFormat(accessor.componentType, size), index);
}
function getIndexFormat(type) {
  switch (type) {
    case o3Core.DataType.UNSIGNED_BYTE:
      return o3Core.IndexFormat.UInt8;

    case o3Core.DataType.UNSIGNED_SHORT:
      return o3Core.IndexFormat.UInt16;

    case o3Core.DataType.UNSIGNED_INT:
      return o3Core.IndexFormat.UInt32;
  }
}
function getElementFormat(type, size) {
  if (type == o3Core.DataType.FLOAT) {
    switch (size) {
      case 1:
        return o3Core.VertexElementFormat.Single;

      case 2:
        return o3Core.VertexElementFormat.Vector2;

      case 3:
        return o3Core.VertexElementFormat.Vector3;

      case 4:
        return o3Core.VertexElementFormat.Vector4;
    }
  }

  if (type == o3Core.DataType.UNSIGNED_SHORT) {
    switch (size) {
      case 2:
        return o3Core.VertexElementFormat.UShort2;

      case 4:
        return o3Core.VertexElementFormat.UShort4;
    }
  }
}
/**
...
var glTFDracoMeshCompression = {
  init: function init() {
    if (!decoder) {
      decoder = new o3Draco.DRACODecoder();
    }
  },
  parse: function parse(extension, primitive, gltfPrimitive, gltf, buffers) {
    ...
    return decoder.decode(buffer, taskConfig).then(function (geometry) {
      var h = 0;

      for (var i = 0; i < geometry.attributes.length; i++) {
        var attribute = geometry.attributes[i];
        var accessorIdx = gltfPrimitive.attributes[attribute.name];
      ...
      var positionAccessorIdx = gltfPrimitive.attributes.POSITION;
      var positionAccessor = accessors[positionAccessorIdx];
      primitive.vertexCount = positionAccessor.count; // load indices
...
var getDefaultMaterial = function () {
  ...
  return function () {
    if (!defaultMateril) {
      defaultMateril = new o3Core.ConstantMaterial("default");
      defaultMateril.emission = new o3Math.Vector4(0.749, 0.749, 0.749, 1);
    }
...
var extensionParsers = {
  KHR_lights: KHR_lights,
  KHR_materials_unlit: o3Core.PBRMaterial,
  // Also have other materials
  KHR_materials_pbrSpecularGlossiness: o3Core.PBRMaterial,
  KHR_techniques_webgl: o3Core.Material,
    ...
    if (RegistedObjs[name] === undefined) {
      ...
      switch (name) {
        case HandledExtensions.PBRMaterial:
          extensionParsers.KHR_materials_unlit = o3Core.PBRMaterial;
          break;
        ...
        default:
          if (o3Core.Material.isPrototypeOf(extobj[name]) && extobj[name].TECH_NAME) RegistedCustomMaterials[extobj[name].TECH_NAME] = extobj[name];
          break;
      }
    }
  });
...
function parseExtensions(resources) {
  ...
  if (extensionsUsed) {
    o3Core.Logger.info("extensionsUsed: ", extensionsUsed);
    ...
    for (var i = 0; i < extensionsUsed.length; i++) {
      if (Object.keys(extensionParsers).indexOf(extensionsUsed[i]) > -1) {
        if (!extensionParsers[extensionsUsed[i]]) {
          o3Core.Logger.warn("extension " + extensionsUsed[i] + " is used, you can add this extension into gltf");
        }
      } else {
        o3Core.Logger.warn("extensionsUsed has unsupported extension " + extensionsUsed[i]);
      }
    }
  }

  if (extensionsRequired) {
    o3Core.Logger.info("extensionsRequired: ".concat(extensionsRequired));
    ...
    for (var _i = 0; _i < extensionsRequired.length; _i++) {
      if (Object.keys(extensionParsers).indexOf(extensionsRequired[_i]) < 0 || !extensionParsers[extensionsRequired[_i]]) {
        o3Core.Logger.error("model has not supported required extension ".concat(extensionsRequired[_i]));
      }
...
function parseResources(resources, name, handler) {
  ...
  if (gltf.hasOwnProperty(name)) {
    ...
    o3Core.Logger.debug(name + ":", entities);
    var promises = [];
...
function parseMaterial(gltfMaterial, resources) {
  ...
  if (gltf.isGltf2 && typeof gltfMaterial.technique === "undefined") {
    ...
    if (pbrMetallicRoughness) {
      ...
      if (baseColorFactor) {
        uniformObj.baseColorFactor = _construct(o3Math.Vector4, _toConsumableArray(baseColorFactor));
      }
    ...
    if (extensions) {
      ...
      if (extensions.KHR_materials_pbrSpecularGlossiness) {
        ...
        if (diffuseFactor) {
          uniformObj.baseColorFactor = _construct(o3Math.Vector4, _toConsumableArray(diffuseFactor));
        }
        ...
        if (specularFactor) {
          uniformObj.specularFactor = _construct(o3Math.Vector3, _toConsumableArray(specularFactor));
        }
    ...
    material = new o3Core.PBRMaterial(gltfMaterial.name || o3Core.PBRMaterial.MATERIAL_NAME, _extends({}, uniformObj, stateObj));
  } else {
    var techniqueName = gltfMaterial.technique;
    o3Core.Logger.warn("Deprecated: Please use a model that meets the glTF 2.0 specification");
    var MaterialType = RegistedCustomMaterials[techniqueName];
  ...
  if (gltfMaterial.hasOwnProperty("values")) {
    ...
    for (var paramName in gltfMaterial.values) {
      if (!material.technique) {
        o3Core.Logger.warn("Cant not find technique");
        continue;
      }

      var uniform = findByKeyValue(material.technique.uniforms, "paramName", paramName);

      if (!uniform) {
        o3Core.Logger.warn("Cant not find uniform: " + paramName);
        continue;
      }
      ...
      if (type === o3Core.DataType.SAMPLER_2D) {
        var textureIndex = gltfMaterial.values[paramName];

        if (o3Core.Util.isArray(textureIndex)) {
          textureIndex = textureIndex[0];
        }

        var texture = getItemByIdx("textures", textureIndex, resources, false);
...
function parseSkin(gltfSkin, resources) {
  ...
  var skin = new o3Core.Skin(gltfSkin.name); // parse IBM
  ...
  for (var i = 0; i < jointCount; i++) {
    ...
    skin.inverseBindMatrices[i] = _construct(o3Math.Matrix, _toConsumableArray(buffer.subarray(startIdx, endIdx)));
  } // get joints
...
function parsePrimitiveVertex(mesh, primitive, gltfPrimitive, gltf, buffers, resources) {
  // load vertices
  var i = 0;
  ...
  for (var attributeSemantic in gltfPrimitive.attributes) {
    ...
    var bufferData = getAccessorData(gltf, accessor, buffers);
    var vertexBuffer = new o3Core.VertexBuffer(resources.engine, bufferData.byteLength, o3Core.BufferUsage.Static);
    vertexBuffer.setData(bufferData);
    primitive.setVertexBufferBindings(new o3Core.VertexBufferBinding(vertexBuffer, stride), i++); // compute bounds

    if (vertexELement.semantic == "POSITION") {
      var position = new o3Math.Vector3();
  ...
  var indexData = getAccessorData(gltf, indexAccessor, buffers);
  var indexCount = indexAccessor.count;
  var indexFormat = getIndexFormat(indexAccessor.componentType);
  var indexByteSize = indexFormat == o3Core.IndexFormat.UInt32 ? 4 : indexFormat == o3Core.IndexFormat.UInt16 ? 2 : 1;
  var indexBuffer = new o3Core.IndexBuffer(resources.engine, indexCount * indexByteSize, o3Core.BufferUsage.Static);
...
function parsePrimitiveMaterial(primitive, gltfPrimitive, resources) {
  ...
  if (gltfPrimitive.material !== undefined) {
    var material = getItemByIdx("materials", gltfPrimitive.material, resources);
...
function parseMesh(gltfMesh, resources) {
  var gltf = resources.gltf,
      buffers = resources.buffers;
  var mesh = new o3Core.Mesh(gltfMesh.name);
  mesh.type = resources.assetType; // parse all primitives then link to mesh
  ...
  var _loop = function _loop(i) {
    primitivePromises.push(new Promise(function (resolve, reject) {
      ...
      var primitive = new o3Core.Primitive(gltfPrimitive.name || gltfMesh.name || i);
      primitive.type = resources.assetType;
      primitive.primitiveTopology = gltfPrimitive.mode == null ? o3Core.PrimitiveTopology.Triangles : gltfPrimitive.mode;
...
function parseAnimation(gltfAnimation, resources) {
  ...
  var animationClip = new o3Core.AnimationClip(gltfAnimation.name || "Animation".concat(animationIdx));
  var duration = -1;
  ...
  for (var i = 0; i < gltfSamplers.length; i++) {
    ...
    var samplerInterpolation = o3Core.InterpolationType.LINEAR;

    switch (gltfSampler.interpolation) {
      case "CUBICSPLINE":
        samplerInterpolation = o3Core.InterpolationType.CUBICSPLINE;
        break;

      case "STEP":
        samplerInterpolation = o3Core.InterpolationType.STEP;
        break;
    }
  ...
  for (var _i6 = 0; _i6 < gltfChannels.length; _i6++) {
    var gltfChannel = gltfChannels[_i6];
...
function parseNode(gltfNode, resources) {
  // TODO: undefined name?
  var entity = new o3Core.Entity(gltfNode.name || "GLTF_NODE_".concat(nodeCount++), resources.engine);

  if (gltfNode.hasOwnProperty("matrix")) {
    var m = gltfNode.matrix;
    var mat = new o3Math.Matrix();
    mat.setValueByArray(m);
    var pos = new o3Math.Vector3();
    var scale = new o3Math.Vector3(1, 1, 1);
    var rot = new o3Math.Quaternion();
...
function buildSceneGraph(resources) {
  ...
  for (var i = gltfNodes.length - 1; i >= 0; i--) {
    ...
    if (gltfNode.hasOwnProperty("mesh")) {
      // find mesh
      var mesh = getItemByIdx("meshes", gltfNode.mesh, resources);
      ...
      if (gltfNode.hasOwnProperty("skin") || mesh.hasOwnProperty("weights")) {
        ...
        node.addComponent(o3Core.SkinnedMeshRenderer, {
          skin: skin,
          mesh: mesh,
          weights: weights
        });
      } else {
        node.addComponent(o3Core.MeshRenderer, {
          mesh: mesh
        });
      }
    }
  ...
  if (nodes.length === 1) {
    ...
    var rootNode = new o3Core.Entity(null, resources.engine);

    for (var _i7 = 0; _i7 < nodes.length; _i7++) {
      rootNode.addChild(nodes[_i7]);
    }

    asset.defaultSceneRoot = rootNode;
  }

  var animator = asset.defaultSceneRoot.addComponent(o3Core.Animation);
...
var GLTFLoader = (_dec$1 = o3Core.resourceLoader(o3Core.AssetType.Perfab, ["gltf", "glb"]), _dec$1(_class$1 = (_temp = /*#__PURE__*/function (_Loader) {
        ...
        return Promise.all(gltf.images.map(function (_ref2) {
          ...
          if (uri) {
            ...
            return resourceManager.load({
              ...
              type: o3Core.AssetType.Texture2D
            });
          } else {
            // 使用 bufferView
            ...
            return loadImageBuffer(bufferData, mimeType).then(function (image) {
              var tex = new o3Core.Texture2D(image.width, image.height, undefined, undefined, resourceManager.engine);
  ...
  _createClass(GLTFLoader, [{
    ...
    value: function load(item, resourceManager) {
      ...
      return new o3Core.AssetPromise(function (resolve, reject) {
  ...
  return GLTFLoader;
}(o3Core.Loader), _temp)) || _class$1);

var _dec$2, _class$2;
var JSONLoader = (_dec$2 = o3Core.resourceLoader(o3Core.AssetType.JSON, ["json"], false), _dec$2(_class$2 = /*#__PURE__*/function (_Loader) {
...
function getEngineFormat(internalFormat) {
  switch (internalFormat) {
    ...
    // break;
    case o3Core.GLCompressedTextureInternalFormat.RGB_S3TC_DXT1_EXT:
      return o3Core.TextureFormat.DXT1;

    case o3Core.GLCompressedTextureInternalFormat.RGBA_S3TC_DXT5_EXT:
      return o3Core.TextureFormat.DXT5;

    case o3Core.GLCompressedTextureInternalFormat.RGB_ETC1_WEBGL:
      return o3Core.TextureFormat.ETC1_RGB;

    case o3Core.GLCompressedTextureInternalFormat.RGB8_ETC2:
      return o3Core.TextureFormat.ETC2_RGB;

    case o3Core.GLCompressedTextureInternalFormat.RGB8_PUNCHTHROUGH_ALPHA1_ETC2:
      return o3Core.TextureFormat.ETC2_RGBA5;

    case o3Core.GLCompressedTextureInternalFormat.RGBA8_ETC2_EAC:
      return o3Core.TextureFormat.ETC2_RGBA8;

    case o3Core.GLCompressedTextureInternalFormat.RGB_PVRTC_2BPPV1_IMG:
      return o3Core.TextureFormat.PVRTC_RGB2;

    case o3Core.GLCompressedTextureInternalFormat.RGBA_PVRTC_2BPPV1_IMG:
      return o3Core.TextureFormat.PVRTC_RGBA2;

    case o3Core.GLCompressedTextureInternalFormat.RGB_PVRTC_4BPPV1_IMG:
      return o3Core.TextureFormat.PVRTC_RGB4;

    case o3Core.GLCompressedTextureInternalFormat.RGBA_PVRTC_4BPPV1_IMG:
      return o3Core.TextureFormat.PVRTC_RGBA4;

    case o3Core.GLCompressedTextureInternalFormat.RGBA_ASTC_4X4_KHR:
      return o3Core.TextureFormat.ASTC_4x4;

    case o3Core.GLCompressedTextureInternalFormat.RGBA_ASTC_5X5_KHR:
      return o3Core.TextureFormat.ASTC_5x5;

    case o3Core.GLCompressedTextureInternalFormat.RGBA_ASTC_6X6_KHR:
      return o3Core.TextureFormat.ASTC_6x6;

    case o3Core.GLCompressedTextureInternalFormat.RGBA_ASTC_8X8_KHR:
      return o3Core.TextureFormat.ASTC_8x8;

    case o3Core.GLCompressedTextureInternalFormat.RGBA_ASTC_10X10_KHR:
      return o3Core.TextureFormat.ASTC_10x10;

    case o3Core.GLCompressedTextureInternalFormat.RGBA_ASTC_12X12_KHR:
      return o3Core.TextureFormat.ASTC_12x12;

    default:
      var formatName = o3Core.GLCompressedTextureInternalFormat[internalFormat];
...
var _dec$3, _class$3;
var KTXCubeLoader = (_dec$3 = o3Core.resourceLoader(o3Core.AssetType.KTXCube, []), _dec$3(_class$3 = /*#__PURE__*/function (_Loader) {
    ...
    value: function load(item, resourceManager) {
      ...
      return new o3Core.AssetPromise(function (resolve, reject) {
        Promise.all(item.urls.map(function (url) {
          ...
          var texture = new o3Core.TextureCubeMap(width, engineFormat, mipmap, resourceManager.engine);
          ...
          for (var face = 0; face < 6; face++) {
            ...
            for (var miplevel = 0; miplevel < length; miplevel++) {
              ...
              texture.setPixelBuffer(o3Core.TextureCubeFace.PositiveX + face, _data, miplevel, 0, 0, _width, height);
            }
          }
...
}(o3Core.Loader)) || _class$3);

var _dec$4, _class$4;
var KTXLoader = (_dec$4 = o3Core.resourceLoader(o3Core.AssetType.KTX, ["ktx"]), _dec$4(_class$4 = /*#__PURE__*/function (_Loader) {
    ...
    value: function load(item, resourceManager) {
      ...
      return new o3Core.AssetPromise(function (resolve, reject) {
        _this.request(item.url, _objectSpread2(_objectSpread2({}, item), {}, {
          ...
          var texture = new o3Core.Texture2D(width, height, engineFormat, mipmap, resourceManager.engine);
  ...
  return KTXLoader;
}(o3Core.Loader)) || _class$4);

var _dec$5, _class$5;
var Texture2DLoader = (_dec$5 = o3Core.resourceLoader(o3Core.AssetType.Texture2D, ["png", "jpg", "webp"]), _dec$5(_class$5 = /*#__PURE__*/function (_Loader) {
    ...
    value: function load(item, resourceManager) {
      ...
      return new o3Core.AssetPromise(function (resolve, reject) {
        _this.request(item.url, _objectSpread2(_objectSpread2({}, item), {}, {
          ...
          var texture = new o3Core.Texture2D(image.width, image.height, undefined, undefined, resourceManager.engine);
...
}(o3Core.Loader)) || _class$5);

var _dec$6, _class$6;
var TextureCubeLoader = (_dec$6 = o3Core.resourceLoader(o3Core.AssetType.TextureCube, [""]), _dec$6(_class$6 = /*#__PURE__*/function (_Loader) {
    ...
    value: function load(item, resourceManager) {
      ...
      return new o3Core.AssetPromise(function (resolve, reject) {
        Promise.all(item.urls.map(function (url) {
          ...
          var tex = new o3Core.TextureCubeMap(width, undefined, undefined, resourceManager.engine);
          if (!tex._glTexture) return;

          for (var faceIndex = 0; faceIndex < 6; faceIndex++) {
            tex.setImageSource(o3Core.TextureCubeFace.PositiveX + faceIndex, images[faceIndex], 0);
          }
...
var GLTFModel = /*#__PURE__*/function (_Component) {
    ...
    set: function set(value) {
      ...
      if (!this._hasBuiltNode) {
        ...
        if (value !== null) {
          ...
          this._animator = this.GLTFNode.getComponent(o3Core.Animation);
  ...
  function GLTFModel(entity, props) {
    var _this;
    ...
    if (isClone) {
      var rootName = _this._props.gltfRootName;

      if (rootName) {
        _this.GLTFNode = _this.entity.findByName(rootName);
      }
    }
    ...
    if (!_this.GLTFNode) {
      ...
      _this._props.gltfRootName = _rootName;
      _this.GLTFNode = _this.entity.createChild(_rootName);
      _this._hasBuiltNode = false;
    } else {
      _this._hasBuiltNode = true;
    }
    ...
    _this.addEventListener("enabled", function () {
      _this.GLTFNode.isActive = true;
    });

    _this.addEventListener("disabled", function () {
      _this.GLTFNode.isActive = false;
    });
...
var compressedTextureLoadOrder = {
  astc: 1,
  s3tc: 2,
  pvrtc: 3,
  etc: 4,
    ...
    value: function update(key, value) {
      if (isAsset(value)) {
        ...
        if (resource) {
          ...
          o3Core.Logger.warn("SchemaResource: ".concat(this.meta.name, " can't find asset, which id is: ").concat(value.id));
        }
      } else {
  ...
  _createClass(TextureResource, [{
    ...
    value: function load(resourceManager, assetConfig, oasis) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (_this.resourceManager.useCompressedTexture && (assetConfig === null || assetConfig === void 0 ? void 0 : (_assetConfig$props = assetConfig.props) === null || _assetConfig$props === void 0 ? void 0 : (_assetConfig$props$co = _assetConfig$props.compression) === null || _assetConfig$props$co === void 0 ? void 0 : _assetConfig$props$co.compressions.length)) {
          var rhi = oasis.engine._hardwareRenderer;
          var compressions = assetConfig.props.compression.compressions;
          compressions.sort(function (a, b) {
          ...
          for (var i = 0; i < compressions.length; i++) {
            ...
            if (compression.container === "ktx" && rhi.canIUse(o3Core.GLCapabilityType[compression.type])) {
              url = compression.url;
              break;
            }
          }
        ...
        resourceManager.load(url).then(function (res) {
          _this._resource = res;
          resolve(_this);
        })["catch"](function (e) {
          reject(e);
  ...
  _createClass(PBRMaterialResource, [{
    ...
    value: function load(resourceManager, assetConfig) {
      ...
      return new Promise(function (resolve) {
        var assetObj = new o3Core.PBRMaterial(assetConfig.name);
    ...
    value: function loadWithAttachedResources(resourceManager, assetConfig) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (assetConfig.resource instanceof o3Core.PBRMaterial) {
        ...
        if (loadPromise) {
          loadPromise.then(function () {
            ...
            getAllGetters(_this3._resource).forEach(function (attr) {
              if (!(material[attr] instanceof o3Core.Texture)) return;
      ...
      Object.keys(this.configProps).forEach(function (attr) {
        ...
        if (isAsset(value)) {
          ...
          if (textureResource && textureResource instanceof TextureResource) {
            ...
            resource[attr] = null;
            o3Core.Logger.warn("PBRMaterialResource: ".concat(_this5.meta.name, " can't find asset \"").concat(attr, "\", which id is: ").concat(value.id));
          }
        } else {
          if (attr === "side") {
            return;
...
var GLTFResource$1 = /*#__PURE__*/function (_SchemaResource) {
  ...
  _createClass(GLTFResource, [{
    ...
    value: function bindMaterials(materials) {
      ...
      var meshes = gltf.meshes; // 兼容material克隆时期生成的schema
      // 通过schema中material数量和gltf中materials数量比较
      // 如果不相等说明是老版本，虽然不准确

      if (materials.length !== gltf.materials.length) {
        for (var i = 0; i < materials.length; i++) {
          var matResource = this.resourceManager.get(materials[i].id);
        ...
        for (var _i = 0; _i < meshes.length; _i++) {
          for (var j = 0; j < meshes[_i].primitives.length; j++) {
            var attachedResource = this.resourceManager.get(materials[index].id);

            if (attachedResource) {
              this._attachedResources.push(attachedResource);

              meshes[_i].primitives[j].material = attachedResource.resource;
            }
        ...
        for (var _i2 = 0; _i2 < materials.length; _i2++) {
          var mtlResource = this.resourceManager.get(materials[_i2].id);

          if (mtlResource) {
            this._attachedResources.push(mtlResource);

            gltf.materials[_i2] = mtlResource.resource;
          } else {
            o3Core.Logger.warn("GLTFResource: ".concat(this.meta.name, " can't find asset \"material\", which id is: ").concat(materials[_i2].id));
          }
        }

        for (var _j = 0; _j < meshes.length; _j++) {
          for (var k = 0; k < meshes[_j].primitives.length; k++) {
            if (meshes[_j].primitives[k].materialIndex !== undefined) {
              // 因为gltf模型中的materials是倒叙遍历的，所以这里要这么写
              var _index = gltf.materials.length - 1 - meshes[_j].primitives[k].materialIndex;

              meshes[_j].primitives[k].material = gltf.materials[_index];
            }
          }
        }
      }
...
var ShaderMaterialResource = /*#__PURE__*/function (_SchemaResource) {
  ...
  _createClass(ShaderMaterialResource, [{
    ...
    value: function createMaterial() {
      var material = new o3Core.ShaderMaterial(this.meta.name || "shader_mtl");
...
var BlinnPhongMaterialResource = /*#__PURE__*/function (_SchemaResource) {
  ...
  _createClass(BlinnPhongMaterialResource, [{
    ...
    value: function load(resourceManager, assetConfig) {
      ...
      return new Promise(function (resolve) {
        var assetObj = new o3Core.BlinnPhongMaterial(assetConfig.name);
  ...
  _createClass(TextureCubeMapResource, [{
    ...
    value: function load(resourceManager, assetConfig, oasis) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var imageUrls = [];
        var type = o3Core.AssetType.TextureCube;
        ...
        if (_this.resourceManager.useCompressedTexture && (assetConfig === null || assetConfig === void 0 ? void 0 : (_assetConfig$props = assetConfig.props) === null || _assetConfig$props === void 0 ? void 0 : (_assetConfig$props$co = _assetConfig$props.compression) === null || _assetConfig$props$co === void 0 ? void 0 : _assetConfig$props$co.compressions.length)) {
          ...
          for (var i = 0; i < compressions.length; i++) {
            ...
            if (compression.container === "ktx" && rhi.canIUse(o3Core.GLCapabilityType[compression.type])) {
              ...
              type = o3Core.AssetType.KTXCube;
              break;
            }
          }
        }

        if (type === o3Core.AssetType.TextureCube) {
  ...
  _createClass(AbilityManager, [{
    ...
    value: function add(abilityConfig) {
      ...
      if (!AbilityConstructor) {
        o3Core.Logger.error("".concat(type, " abiltiy is not defined"));
        return;
      }

      var abilityProps = this.mixPropsToExplicitProps(props); // if (AbilityConstructor === Camera) {
      //   abilityProps.SceneRenderer = o3.BasicRenderPipeline;
      // }

      var ability = node.addComponent(AbilityConstructor, abilityProps);
      ...
      for (var k in props) {
        ...
        if (prop && this.checkIsAsset(prop)) {
          ...
          if (res) {
            ...
            o3Core.Logger.warn("AbilityManager: can't get asset \"".concat(k, "\", which id is ").concat(prop.id));
          }
        }
      }
...
var NodeManager = (_dec$8 = pluginHook({
  ...
  function NodeManager(oasis) {
    ...
    this.nodeMap = {};
    this.root = void 0;
    this.root = new o3Core.Entity("root", this.oasis.engine);
  ...
  _createClass(NodeManager, [{
    ...
    value: function create(nodeConfig) {
      ...
      var entity = new o3Core.Entity(name, this.oasis.engine);
      entity.isActive = isActive;
      entity.transform.position = new o3Math.Vector3(position[0], position[1], position[2]);
      entity.transform.rotation = new o3Math.Vector3(rotation[0], rotation[1], rotation[2]);
      entity.transform.scale = new o3Math.Vector3(scale[0], scale[1], scale[2]);
      entity.id = id;
...
var Oasis = (_dec$a = pluginHook({
  ...
  _createClass(Oasis, [{
    ...
    value: function loadResources() {
      ...
      var loadingPromises = Object.values(assets).filter(function (asset) {
  ...
  return Oasis;
}(o3Core.EventDispatcher), _temp$4), (_applyDecoratedDescriptor(_class$a.prototype, "init", [_dec$a], Object.getOwnPropertyDescriptor(_class$a.prototype, "init"), _class$a.prototype)), _class$a));
  ...
  for (var i = 0, l = keys.length; i < l; ++i) {
    ...
    if (v !== null && _typeof(v) === "object" && v.length > 1) {
      if (k === "backgroundColor" || k === "tintColor") {
        props[k] = new o3Math.Vector4(v[0], v[1], v[2], v[3]);
      } else if (_vec3Attribute.indexOf(k) !== -1) {
        props[k] = new o3Math.Vector3(v[0], v[1], v[2]);
      }
    }
  }
}
...
function handleAssets() {
  ...
  for (var i = 0, l = keys.length; i < l; i++) {
    ...
    switch (value === null || value === void 0 ? void 0 : value.length) {
      case 2:
        props[key] = new o3Math.Vector2(value[0], value[1]);
        break;

      case 3:
        props[key] = new o3Math.Vector3(value[0], value[1], value[2]);
        break;

      case 4:
        props[key] = new o3Math.Vector4(value[0], value[1], value[2], value[3]);
        break;
    }
  }
}
