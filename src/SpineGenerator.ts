import { Texture2D, SubPrimitive, Vector3, Material, Engine, BoundingBox } from "@galacean/engine";
import {
  Skeleton,
  SkeletonClipping,
  RegionAttachment,
...
export class SpineGenerator {
  ...
  buildPrimitive(skeleton: Skeleton, renderer: SpineAnimationRenderer) {
    ...
    const { engine, _indices, _vertices, _vertexCount, _subPrimitives, zSpacing, premultipliedAlpha } = renderer;
    let { tempVerts, tempTexture, tempBlendMode, subRenderItemPool, subPrimitivePool } = SpineGenerator;
    ...
    for (let slotIndex = 0; slotIndex < maxSlotCount; ++slotIndex) {
      ...
      if (texture != null) {
        ...
        if (isClipping) {
          _clipper.clipTriangles(tempVerts, triangles, triangles.length, uvs, finalColor, dark, false);
        ...
        const blendModeChanged = tempBlendMode !== null && tempBlendMode !== slotData.blendMode;
