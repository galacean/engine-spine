 ...
 */
export function quickSort<T>(a: T[], from: number, to: number, compareFunc: Function): void {
  while (true) {
    // Insertion sort is faster for short arrays.
    if (to - from <= 10) {
      _insertionSort(a, from, to, compareFunc);
      return;
    }
...
function _insertionSort<T>(a: T[], from: number, to: number, compareFunc: Function): void {
